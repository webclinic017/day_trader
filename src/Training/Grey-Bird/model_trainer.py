...
class ModelTrainer():
    ...
    def save_state(self, model: object, target_model: object, it_num: int, replay_mem: deque, X: list, Y: list, ver: int) -> None:
    ...
    def simulate(self, env: TrainingEnviroment, decay: float, ver: int) -> None:
        ...
        self.models = model.Model(84, 3)
        self.models.target_model.set_weights(model.get_weights())
        ...
        for i in tqdm(range(1000)):
            ...
            env.reset(-1)
            print(f"    Goal profit of: {env.goal_profit}, starting price: {env.closing_prices[env.curr_chunk]}, ending price: {env.closing_prices[env.curr_chunk+env.week]}")
            ...
            while(not done):
                ...
                if not done and reset_num > 0:  # Didn't meet quota so resetting on the week
                    ...
                    if num_it > 9 and num_it % 10 == 0:
                        avg = round(total_money / 10, 2)
                        total_money = 0
            ...
            self.save_state(self.models.model, self.models.target_model, i , replay_memory, X, y, ver)     
