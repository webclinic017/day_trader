...
class Model():
    ...
    def __init__(self, *args) -> None:
        ...
        if not args:
            ...
            self.model = self.create_agent(args[0][0], args[0][1])
            self.target_model = self.create_agent(args[0][0], args[0][1])

            self.first_layer = args[0][0]
            self.second_layer = args[0][1]



    def create_agent(self, first_layer: int, second_layer: int) -> object:
        ...
        model.add(LSTM(first_layer, input_shape=(1, 93), activation='relu', return_sequences=True))
        model.add(keras.layers.Dropout(0.25))

        model.add(LSTM(second_layer, activation='relu', return_sequences=True))
